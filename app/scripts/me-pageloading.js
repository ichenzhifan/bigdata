angular.module('me-pageloading', [])
    ...
    .factory('mePageLoading',['Animater','mePageLoadingTemplate','$timeout',
        function(Animater,mePageLoadingTemplate,$timeout){
        var animaterArr = {},
        service = {
          registerAnimater:function(id,el,effect){
            var wrapperDiv = document.createElement('div'),loaderElement;
            wrapperDiv = angular.element(wrapperDiv).html(mePageLoadingTemplate);
            loaderElement = wrapperDiv.children();
            angular.element(el).append(loaderElement);

            animaterArr[id] = new Animater({
              el:loaderElement,
              effect:effect
            })
          },
          removeAnimater:function(id){
            animaterArr[id] = null;
          },
          show:function(id,effect){
            animaterArr[id] && animaterArr[id].show(effect)
          },
          hide:function(id){
            animaterArr[id] && ($timeout(function() {
                animaterArr[id].hide();
            }, 500));
    ...
    .run(['$rootScope', 'mePageLoading',
        function($rootScope, mePageLoading) {
            ...
            changeStart = function(e,config) {
                if(config.params && config.params.animateId){
                    mePageLoading.show(config.params.animateId);
                }
            };

            changeSuccess = changeError = function(e,config) {
                if(config.params && config.params.animateId){
                    mePageLoading.hide(config.params.animateId);
                }
            };
