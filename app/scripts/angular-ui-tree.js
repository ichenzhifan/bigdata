    ...
    .controller('TreeNodeController', ['$scope', '$element',
      function ($scope, $element) {
        ...
        $scope.toggle = function (node) {
          ...
          if($scope.collapsed){
            var $tar = $element.children('ol')
            $tar.stop().addClass('hiddenover').css('height','auto')
            $tar.css('height',$tar.height()+'px')
            $tar.animate({height:"0px"},200)
            if(showtm){clearTimeout(showtm);showtm = null;}
            showtm = setTimeout(function(){
              $tar.addClass('hidden')
              $tar.removeClass('hiddenover')
              $tar.css('height','auto')
              showtm = null
            },500)
          }else{
            var $tar = $element.children('ol'),height
            $tar.stop().removeClass('hidden').css('height','auto')
            height = $tar.height()
            $tar.css('height','0px')
            $tar.animate({height:height+"px"},200)
            if(showtm){clearTimeout(showtm);showtm = null;}
            showtm = setTimeout(function(){
              $tar.css('height','auto')
              showtm = null
